...
ssf_config = {'alpha1':0.018660,
              'alpha1_sigma':1.512691,
              'alpha2':0.048385,
              'alpha2_sigma':7.251255,
              'n1':1.239299,
              'n1_sigma':0.089600,
              'n2':1.495341,
              'n2_sigma':0.467717}
...
wmf_config = {'alpha1':0.015731,
              'alpha1_sigma':1.479686,
              'alpha2':0.080320,
              'alpha2_sigma':1.764580,
              'n1':1.910677,
              'n1_sigma':0.313703,
              'n2':2.305446,
              'n2_sigma':0.132053}

def genDist(mu0, sigma0, mu1, sigma1, nsample=1000, log=False, weight=0.5):
    # nsample - number of samples 
    # log - set to true if distribution made in log space 
    # weight - how much weight to put on mu0 
    assert isinstance(nsample, int)
    n0 = int(nsample*weight)
    n1 = nsample - n0 
    if log: 
        s0 = 10**np.random.normal(np.log10(mu0), np.log10(sigma0), n0)
        s1 = 10**np.random.normal(np.log10(mu1), np.log10(sigma1), n1)
    else:
        s0 = np.random.normal(mu0, sigma0, n0)
        s1 = np.random.normal(mu1, sigma1, n1)
    distribution = np.concatenate([s0,s1])
    
    return distribution 

ssf_a = genDist(
    ssf_config['alpha1'], 
    ssf_config['alpha1_sigma'], 
    ssf_config['alpha2'], 
    ssf_config['alpha2_sigma'], 
    samples,
    True,
    0.32
    )
...
wmf_a = genDist(
    wmf_config['alpha1'], 
    wmf_config['alpha1_sigma'], 
    wmf_config['alpha2'], 
    wmf_config['alpha2_sigma'], 
    samples,
    True,
    0.40
    )
...
nbin = 100 
axs[0,0].hist(np.log10(ssf_a), bins=nbin, density=True)
axs[1,0].hist(ssf_n, bins=nbin, density=True)
axs[0,1].hist(np.log10(wmf_a), bins=nbin, density=True)
...
axs[0,0].set_xticks(xlabel_pos, xlabel_val)
axs[0,1].set_xticks(xlabel_pos, xlabel_val)
...
# matrix is | element saturation | time step | model run  
