...
SSF = material(Ksat=0.11,theta_res=0.06,theta_sat=0.38,
               alpha=0.000325,vn=1.82,name='STAITHES')
#parameters taken from coupled approach 
...
## create handler
h = handler(dname='HydroBestFit', ifac=1,tlength=secinday,iobs=100, 
            flow = 'transient',
            transport = 'transient',
            sim_type='solute')
h.maxIter = 300
...
step['zone'] = zone_node 
step.to_csv(os.path.join(h.dname,'frun.csv'),index=False)
