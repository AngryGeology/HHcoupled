...
from SUTRAhandler import handler, material, secinday  
...
SSF = material(Ksat=0.11,theta_res=0.06,theta_sat=0.38,
               alpha=1.317e-4,vn=2.5,name='STAITHES')
WMF = material(Ksat=0.013,theta_res=0.1,theta_sat=0.48,
               alpha=0.126e-4/2,vn=1.44,name='WHITBY')
DOG = material(Ksat=0.309,theta_res=0.008,theta_sat=0.215,
               alpha=1.27e-4,vn=1.45,name='DOGGER')
RMF = material(Ksat=0.013,theta_res=0.1,theta_sat=0.48,
               alpha=0.126e-4,vn=1.44,name='REDCAR')

#%% step 3, setup handler 
## create handler
h = handler(dname='HydroModel', ifac=1,tlength=secinday,iobs=100, 
            flow = 'transient',
            transport = 'transient',
            sim_type='solute')
h.maxIter = 300
...
infil = (rain/100)/secinday #in kg/s # check this #### 
...
#%% step 7, setup initial conditions (read from warmup run)
pres = warmup['Pressure'].values 

# following calculation are somewhat redundant 
...
pressure_vals = None # right_pressure_val 
